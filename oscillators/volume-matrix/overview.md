---
icon: list-ol
---

# Overview

The **Volume Matrix** is a comprehensive market analysis tool designed to help traders identify the invisible shifts in supply and demand. In a market where price action can often be misleading, the Volume Matrix acts as a filter, highlighting the moments when price movement is supported by significant volume and when it is merely "market noise."

{% content-ref url="volume-line.md" %}
[volume-line.md](volume-line.md)
{% endcontent-ref %}

{% content-ref url="overbought-and-oversold.md" %}
[overbought-and-oversold.md](overbought-and-oversold.md)
{% endcontent-ref %}

{% content-ref url="volatility-bands.md" %}
[volatility-bands.md](volatility-bands.md)
{% endcontent-ref %}

{% content-ref url="alerts.md" %}
[alerts.md](alerts.md)
{% endcontent-ref %}

Standard oscillators like the RSI or Stochastic often provide signals based solely on price, which can lead to frequent false alarms during low-volume periods. The Volume Matrix solves this by weighting every calculation with trade volume. This means the indicator remains stable during quiet periods and becomes highly responsive when the "Big Money" enters the market.
